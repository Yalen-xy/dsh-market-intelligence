...
export function marketState(now, closures) {
...
export function shanghaiMinuteOfDay(now) {
    const parts = shanghaiParts(now);
...
export function isMarketTradingDate(tradingDate, market, closures) {
    ...
    if (represented.getUTCFullYear() !== year
...
export function nextStateChange(now, market, closures) {
    const parts = shanghaiParts(now);
...
export function lastClosedTradingDate(now, market, closures) {
    const parts = shanghaiParts(now);
...
function sessionFor(market, parts, closures) {
    const tradingDate = dateKey(parts.year, parts.month, parts.day);
    const calendarConfidence = Object.hasOwn(closures, String(parts.year)) ? 'configured' : 'degraded';
    if (!isTradingDay(parts, market, closures)) {
    ...
    const phase = minute >= lunchStart && minute < lunchEnd ? 'lunch' : 'closed';
    return { phase, calendarConfidence, tradingDate, active: false };
}
function nextTradingDate(parts, market, closures) {
    ...
    for (let offset = 1; offset <= 370; offset++) {
        ...
        if (isTradingDay(candidate, market, closures))
            return candidate;
    }
    throw new Error(`No ${market} trading day found within one year`);
}
function previousTradingDate(parts, market, closures) {
    const cursor = new Date(Date.UTC(parts.year, parts.month - 1, parts.day));
    for (let offset = 1; offset <= 370; offset++) {
        cursor.setUTCDate(cursor.getUTCDate() - 1);
        const candidate = {
...
function isTradingDay(parts, market, closures) {
...
function atShanghaiMinute(year, month, day, minute) {
