...
import { loadUserState, mutateWatchlist, } from './config.js';
...
import { createSharedRequestLimiter, startMarketRuntime } from './runtime.js';
...
export function createApply(overrides = {}) {
    ...
    return async function applyWithDependencies(ctx, rawConfig) {
        ...
        return ctx.effect(async () => startLifecycle(ctx, config, dshHome, dependencies), 'market-intelligence lifecycle');
    };
}
...
async function startLifecycle(ctx, config, dshHome, dependencies) {
...
function requireDshHome(value) {
    if (typeof value !== 'string' || value.trim() === '')
        throw new Error('DSH_HOME must be a non-empty normalized absolute local Windows path');
    return requireLocalWindowsPath(value, 'DSH_HOME');
}
export function resolveDshBaseDirectory(environment) {
    return requireDshHome(environment.DSH_HOME);
}
function requireStorageDir(value) {
    if (typeof value !== 'string' || value.trim() === '')
        throw new Error('storageDir must be a non-empty normalized absolute local Windows path');
