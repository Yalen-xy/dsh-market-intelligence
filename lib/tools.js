...
import { assertSupportedJsonSchema, defineTool, ToolArgsError, ToolOutputError, validateJsonSchemaValue, valueSchemaSpecToJsonSchema, } from '@deepseek-ai/dsh-tools';
...
function strictDefinition(definition, parameters) {
...
function projectToolOutput(tool, schema, project) {
    try {
        const value = project();
        const violations = validateJsonSchemaValue(schema, value, '');
        if (violations.length > 0)
            throw new ToolOutputError(tool, violations);
        return value;
    }
    catch (error) {
