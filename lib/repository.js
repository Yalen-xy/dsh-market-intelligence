...
export class MarketRepository {
    ...
    initializeRecoveryCursors(provider, interval, seeds, updatedAt) {
        this.assertOpen();
        ...
        for (const seed of seeds) {
            if (!seed || typeof seed !== 'object')
                throw new Error('recovery cursor seed must be an object');
            validateMarket(seed.market);
            requireTimestamp(seed.cursor, 'recovery seed cursor');
            if (Date.parse(seed.cursor) > Date.parse(updatedAt))
                throw new Error('recovery seed cursor must not follow updatedAt');
        }
      ...
      `);
            for (const seed of seeds)
                insert.run(provider, seed.market, interval, seed.cursor, updatedAt);
        });
    }
    recoveryCursor(provider, market, interval) {
        this.assertOpen();
    ...
    commitRecoverySegment(segment) {
      ...
      `).get(segment.provider, segment.market, segment.interval);
            if (row !== undefined) {
                const cursor = stringColumn(row, 'cursor_at');
                requireTimestamp(cursor, 'recovery cursor');
                if (Date.parse(segment.end) <= Date.parse(cursor))
                    return;
            ...
            if (segment.items.length > 0)
                this.insertQuotes(segment.items);
            if (segment.gap !== null)
                this.recordGap(segment.gap);
    ...
    querySeries(query) {
        ...
        if (!ALLOWED_INTERVALS.has(query.interval))
            throw new Error('interval is unsupported');
        if (query.start !== undefined)
            requireTimestamp(query.start, 'start');
        if (query.end !== undefined)
            requireTimestamp(query.end, 'end');
        if (query.start !== undefined && query.end !== undefined && Date.parse(query.start) >= Date.parse(query.end))
            throw new Error('series start must be before end');
        const limit = validateLimit(query.limit ?? 500);
        const clauses = ['symbol = ?', 'interval = ?'];
        const values = [symbol, query.interval];
        if (query.start !== undefined) {
    ...
    writeSectors(sectors, resolution = 'intraday') {
        this.assertOpen();
    ...
    readSectors(query = {}) {
        ...
        if (query.category !== undefined)
            requireString(query.category, 'category');
        if (query.resolution !== undefined)
            validateSectorResolution(query.resolution);
        const limit = validateLimit(query.limit ?? 500);
        const clauses = [];
        const values = [];
        if (query.category !== undefined) {
    ...
    `).run(update.provider, update.available ? 1 : 0, update.latencyMs, update.lastAttemptAt, update.lastSuccessAt, update.lastFailureAt, update.consecutiveFailures, update.error === null ? null : sanitizeError(update.error));
    }
    health() {
        this.assertOpen();
    ...
    `);
        for (const quote of quotes) {
            statement.run(quote.symbol, quote.name, quote.market, quote.currency, quote.price, quote.open, quote.high, quote.low, quote.previousClose, quote.volume, quote.amount, quote.change, quote.changePercent, quote.marketTime, quote.marketTime ?? quote.fetchedAt, shanghaiDate(quote.marketTime ?? quote.fetchedAt), quote.fetchedAt, quote.source, quote.isDelayed ? 1 : 0, quote.isStale ? 1 : 0);
        }
    }
    insertBars(bars) {
        const statement = this.database.prepare(`
    ...
    `);
        for (const bar of bars)
            statement.run(bar.symbol, bar.market, bar.interval, bar.timestamp, shanghaiDate(bar.timestamp), bar.open, bar.high, bar.low, bar.close, bar.volume, bar.turnover);
    }
    insertSectors(sectors, resolution) {
        const statement = this.database.prepare(`
    ...
    `);
        ...
        for (const sector of sectors) {
            ...
            changed += changeCount(statement.run(sector.id, sector.name, sector.category, sector.changePercent, sector.turnover, sector.netFlow, sector.leaderSymbol, sector.leaderName, sector.leaderChangePercent, sector.marketTime, sector.fetchedAt, sector.source, sector.isDelayed ? 1 : 0, sector.isStale ? 1 : 0, sectorBucket(representedTime, resolution), resolution, representedTime, sectorTieBreaker(sector)));
        }
        return changed;
    }
    listRawTradingDates(market) {
    ...
    deleteRawSymbols(market, tradingDate, symbols) {
        validateMarket(market);
        requireTradingDate(tradingDate);
    ...
    deleteMinuteDate(tradingDate, market) {
        requireTradingDate(tradingDate);
        if (market !== undefined)
            validateMarket(market);
    ...
    recordMaintenance(record) {
        ...
        if (result === undefined)
            throw new Error('maintenance result must be lossless JSON');
        this.database.prepare(`
    ...
    liveDatabaseBytes() {
        ...
        if (!Number.isSafeInteger(bytes))
            throw new Error('live database size exceeds JSON-safe range');
        return bytes;
    }
    databaseBytes() {
        if (this.path === ':memory:')
    ...
    attemptMutation(operation, accept) {
        if (typeof operation !== 'function' || typeof accept !== 'function')
            throw new Error('maintenance mutation requires operation and accept functions');
    ...
    assertOpen() {
        if (this.closed)
            throw new Error('market repository is closed');
    }
}
function migrateSchemaV1ToV2(database) {
    ...
    try {
        ...
        if (!columns.has('resolution'))
            database.exec("ALTER TABLE sector_observations ADD COLUMN resolution TEXT NOT NULL DEFAULT 'intraday' CHECK (resolution IN ('intraday', 'daily'))");
        if (!columns.has('represented_time'))
            database.exec("ALTER TABLE sector_observations ADD COLUMN represented_time TEXT NOT NULL DEFAULT ''");
        if (!columns.has('tie_breaker'))
            database.exec("ALTER TABLE sector_observations ADD COLUMN tie_breaker TEXT NOT NULL DEFAULT ''");
        ...
        if (!barColumns.has('trading_date'))
            database.exec("ALTER TABLE bars ADD COLUMN trading_date TEXT NOT NULL DEFAULT ''");
...
function migrateSchemaV2ToV3(database) {
    database.exec('BEGIN IMMEDIATE');
    try {
        database.exec(RECOVERY_SCHEMA);
        assertRecoverySchemaV3(database);
...
function assertSectorSchemaV2(database) {
    const columns = sectorColumnNames(database);
    for (const required of ['resolution', 'represented_time', 'tie_breaker']) {
        if (!columns.has(required))
            throw new Error(`schema v2 sector_observations is missing ${required}`);
    }
}
function sectorColumnNames(database) {
    return new Set(database.prepare('PRAGMA table_info(sector_observations)').all().map((row) => stringColumn(row, 'name')));
}
function assertBarSchemaV2(database) {
    if (!barColumnNames(database).has('trading_date'))
        throw new Error('schema v2 bars is missing trading_date');
}
function barColumnNames(database) {
    return new Set(database.prepare('PRAGMA table_info(bars)').all().map((row) => stringColumn(row, 'name')));
}
function assertRecoverySchemaV3(database) {
    ...
    for (const required of ['provider', 'market', 'interval', 'cursor_at', 'updated_at']) {
        if (!columns.has(required))
            throw new Error(`schema v3 recovery_progress is missing ${required}`);
    }
}
function validateQuote(quote) {
    ...
    if ((quote.market === 'CN' && quote.currency !== 'CNY') || (quote.market === 'HK' && quote.currency !== 'HKD'))
        throw new Error('quote currency does not match market');
    for (const field of ['price', 'open', 'high', 'low', 'previousClose', 'volume', 'amount', 'change', 'changePercent'])
        requireNullableFinite(quote[field], `quote ${field}`);
    if (quote.marketTime !== null)
        requireTimestamp(quote.marketTime, 'quote marketTime');
...
function validateBar(bar) {
    ...
    if (!ALLOWED_INTERVALS.has(bar.interval))
        throw new Error('bar interval is unsupported');
    requireTimestamp(bar.timestamp, 'bar timestamp');
    for (const field of ['open', 'high', 'low', 'close'])
        requireFinite(bar[field], `bar ${field}`);
...
function validateSector(sector) {
    if (!sector || typeof sector !== 'object')
        throw new Error('sector must be an object');
    requireString(sector.id, 'sector id');
    requireString(sector.name, 'sector name');
    requireString(sector.category, 'sector category');
    for (const field of ['changePercent', 'turnover', 'netFlow', 'leaderChangePercent'])
        requireNullableFinite(sector[field], `sector ${field}`);
    requireNullableString(sector.leaderSymbol, 'sector leaderSymbol');
    requireNullableString(sector.leaderName, 'sector leaderName');
    if (sector.marketTime !== null)
        requireTimestamp(sector.marketTime, 'sector marketTime');
...
function validateGap(gap) {
    ...
    if (gap.symbol !== null)
        validatedCanonicalSymbol(gap.symbol, 'gap symbol', gap.market);
    requireString(gap.interval, 'gap interval');
    requireTimestamp(gap.start, 'gap start');
    requireTimestamp(gap.end, 'gap end');
    if (Date.parse(gap.start) >= Date.parse(gap.end))
        throw new Error('gap start must be before end');
    requireString(gap.reason, 'gap reason');
    requireTimestamp(gap.recordedAt, 'gap recordedAt');
}
function validateRecoveryIdentity(provider, market, interval) {
    requireString(provider, 'recovery provider');
    validateMarket(market);
    if (interval !== 'quote')
        throw new Error('recovery interval must be quote');
}
function validateRecoverySegment(segment) {
    if (!segment || typeof segment !== 'object')
        throw new Error('recovery segment must be an object');
    ...
    if (Date.parse(segment.start) >= Date.parse(segment.end))
        throw new Error('recovery start must be before end');
    requireTimestamp(segment.completedAt, 'recovery completedAt');
    if (Date.parse(segment.completedAt) < Date.parse(segment.end))
        throw new Error('recovery completedAt must not precede end');
    assertDenseArray(segment.items, 'recovery items');
    for (const item of segment.items) {
        validateQuote(item);
        if (item.market !== segment.market)
            throw new Error('recovery item market does not match segment market');
        if (item.source !== segment.provider)
            throw new Error('recovery item source does not match provider');
    ...
    if (segment.gap !== null) {
        ...
        if (segment.gap.market !== segment.market)
            throw new Error('recovery gap market does not match segment market');
        if (segment.gap.symbol !== null)
            throw new Error('recovery gap symbol must be null');
        if (segment.gap.interval !== segment.interval)
            throw new Error('recovery gap interval does not match segment interval');
...
function validateProviderHealth(update) {
    if (!update || typeof update !== 'object')
        throw new Error('provider health must be an object');
    ...
    if (update.latencyMs !== null && update.latencyMs < 0)
        throw new Error('provider latencyMs must be non-negative');
    for (const field of ['lastAttemptAt', 'lastSuccessAt', 'lastFailureAt']) {
        if (update[field] !== null)
            requireTimestamp(update[field], `provider ${field}`);
    }
    if (!Number.isSafeInteger(update.consecutiveFailures) || update.consecutiveFailures < 0)
        throw new Error('provider consecutiveFailures must be a non-negative safe integer');
...
function sectorBucket(timestamp, resolution) {
    if (resolution === 'daily')
        return shanghaiStartOfDay(shanghaiDate(timestamp));
    const milliseconds = Date.parse(timestamp);
    return new Date(Math.floor(milliseconds / 300_000) * 300_000).toISOString();
}
function sectorTieBreaker(sector) {
...
function shanghaiStartOfDay(tradingDate) {
    requireTradingDate(tradingDate);
...
function validateSectorResolution(value) {
    if (value !== 'intraday' && value !== 'daily')
        throw new Error('sector resolution must be intraday or daily');
}
function validatedCanonicalSymbol(value, label, expectedMarket) {
    ...
    if (expectedMarket !== undefined && canonicalized.market !== expectedMarket)
        throw new Error(`${label} does not match market`);
    return canonical;
}
...
function timestampOffsetMinutes(zone) {
    if (zone === 'Z')
        return 0;
...
function requireTradingDate(value) {
    if (typeof value !== 'string' || !TRADING_DATE.test(value) || !isCalendarDate(value))
        throw new Error('trading date must be a valid YYYY-MM-DD date');
}
function isCalendarDate(value) {
    const [year, month, day] = value.split('-').map(Number);
    if (year === undefined || month === undefined || day === undefined || month < 1 || month > 12 || day < 1 || day > daysInMonth(year, month))
        return false;
...
function assertJsonSafe(value, label, active = new WeakSet()) {
    ...
    if (Array.isArray(value)) {
        if (Object.getPrototypeOf(value) !== Array.prototype)
            throw new Error(`${label} contains a class instance`);
        ...
        for (const item of value)
            assertJsonSafe(item, label, active);
        active.delete(value);
        return;
    }
...
function nullableStringColumn(row, column) {
    ...
    if (value === null)
        return null;
    if (typeof value !== 'string')
        throw new Error(`SQLite column ${column} is not a string or null`);
    return value;
}
function numberColumn(row, column) {
    const value = rowRecord(row)[column];
    if (typeof value !== 'number' || !Number.isFinite(value))
        throw new Error(`SQLite column ${column} is not finite`);
    return value;
}
function nullableNumberColumn(row, column) {
    const value = rowRecord(row)[column];
    if (value === null)
        return null;
    if (typeof value !== 'number' || !Number.isFinite(value))
        throw new Error(`SQLite column ${column} is not finite or null`);
    return value;
}
function booleanColumn(row, column) {
    const value = numberColumn(row, column);
    if (value !== 0 && value !== 1)
        throw new Error(`SQLite column ${column} is not boolean`);
    return value === 1;
}
function marketColumn(row, column) {
    const value = stringColumn(row, column);
    validateMarket(value);
    return value;
}
function currencyColumn(row, column) {
    const value = stringColumn(row, column);
    if (value !== 'CNY' && value !== 'HKD')
        throw new Error(`SQLite column ${column} is not a currency`);
    return value;
}
function changeCount(result) {
    const changes = result.changes;
    if (typeof changes === 'bigint') {
        if (changes > BigInt(Number.MAX_SAFE_INTEGER))
            throw new Error('SQLite change count exceeds JSON-safe range');
...
function parseJsonObject(value) {
    ...
    if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object')
        throw new Error('stored maintenance result is not an object');
    return parsed;
}
