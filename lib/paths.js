...
export function requireLocalWindowsPath(value, label, requiredBasename) {
...
function hasUnsafeWindowsComponent(value) {
...
export async function getWindowsDriveType(driveRoot, execFileImpl = execFile) {
    if (!WINDOWS_DRIVE_ROOT.test(driveRoot))
        throw new Error('drive root must be a validated local Windows drive root');
    const output = await new Promise((resolve, reject) => {
        execFileImpl('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', DRIVE_TYPE_SCRIPT], { env: { ...process.env, DSH_MARKET_DRIVE_ROOT: driveRoot } }, (error, stdout) => error === null ? resolve(String(stdout)) : reject(error));
    });
    const trimmedOutput = output.trim();
    if (!/^\d+$/.test(trimmedOutput))
        throw new Error('unable to determine Windows drive type');
    const driveType = Number(trimmedOutput);
    if (!Number.isInteger(driveType))
        throw new Error('unable to determine Windows drive type');
    return driveType;
}
export async function assertSafeLocalWindowsPath(pathValue, dependencies = {}) {
    ...
    if (await getDriveTypeImpl(driveRoot) !== 3)
        throw new Error(`${localPath} must be on a local fixed Windows drive`);
    ...
    for (let current = localPath;; current = path.win32.dirname(current)) {
        try {
            ...
            if (stats.isSymbolicLink())
                throw new Error(`${localPath} must not traverse a reparse point`);
        }
        catch (error) {
            if (!isMissingPath(error))
                throw error;
        }
        if (current === driveRoot)
            return;
    }
}
