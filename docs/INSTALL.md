...
This guide describes the Windows GitHub Release installer as it is implemented. It is for a valid `desktop` profile of the DeepSeek Harness (DSH) host and a host-managed CLI that identifies itself and exposes `plugin add` and `plugin remove`.
...
- A verifiable DeepSeek Harness (DSH) managed CLI with `plugin add/remove` support.
- DeepSeek Harness (DSH) must be closed before mutation. If the DSH host is running from the detected home or managed-command roots, the installer refuses to mutate the profile. It does not stop or restart processes.
...
Normal users should use the fixed-name latest ZIP linked from the [root README](../README.md#deepseek-harness), extract it, and double-click `INSTALL.cmd`. The advanced commands in this guide:
...
3. parses every non-empty manifest line and requires exactly the five current Release checksum rows (`install.ps1`, `uninstall.ps1`, the versioned DSH tgz, `claude-market-intelligence-latest.mcpb`, and `LICENSE.txt`), with lowercase 64-hex hashes, canonical names, no extra/missing rows, no case-insensitive duplicate names, and no duplicate hashes;
...
The CLI is resolved from explicit `-DshCommand`, one valid `dsh` application on `PATH`, or one recognized DSH host application-data location. Ambiguous or unverifiable candidates fail closed.
...
```powershell
& .\install.ps1 -Version '0.2.0' -ReleaseApiUri 'https://api.github.com/repos/Yalen-xy/dsh-market-intelligence/releases/tags/v0.2.0' -AcceptLicense -DshHome 'E:\DSH\data'
& .\install.ps1 -Version '0.2.0' -ReleaseApiUri 'https://api.github.com/repos/Yalen-xy/dsh-market-intelligence/releases/tags/v0.2.0' -AcceptLicense -DshHome 'E:\DSH\data' -WhatIf
```
...
$names = @('install.ps1', 'uninstall.ps1', 'LICENSE.txt', 'dsh-market-intelligence-0.2.0.tgz')
...
Pre-restart validation does not prove runtime registration. Start the DSH host normally, open the installed `desktop` profile, and ask it to list the tools registered by `dsh-market-intelligence`. Verify that these exact seven names are present once each:
